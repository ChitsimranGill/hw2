...
public class Connection
{
   ...
   @param key the phone key pressed by the user
	...
	/**
	 * Method for Accessing Mailbox. It does not get greeting.
	   Respond to the user's pressing a key on the phone touchpad
	   @param key the phone key pressed by the user
		 */
		public void dial2(String key)
		{
			if (state == CONNECTED)
				connect2(key);
			else if (state == RECORDING)
   ...
   @param key the phone key pressed by the user
	...
	/**
	 * Method for Accessing Mailbox. Prints: Please enter your passcode.
	   Try to connect the user with the specified mailbox.
	   @param key the phone key pressed by the user
		 */
		private void connect2(String key)
		{
			if (key.equals("#"))
			{
				currentMailbox = system.findMailbox(accumulatedKeys);
  ...
  @param key the phone key pressed by the user
	...
	private void mailboxMenu(String key)
	{
		...
		else if ("123".contains(key)==false)
		{
			phone.speak(MAILBOX_MENU_TEXT);
		}
	}
  ...
  @param key the phone key pressed by the user
	...
	private void messageMenu(String key)
	{
		...
		else if ("1234".contains(key)==false)
		{
			phone.speak(MESSAGE_MENU_TEXT);
		}
	}
	...
	private int state;
	public static int connectCheck = 0; // to check if connected 
	public static int loginCheck = 0; // to check if logged in
	public static int mailboxCheck = 0; // to check if mailbox is accessed
	public static int recordCheck = 0; //  to check if recorded
